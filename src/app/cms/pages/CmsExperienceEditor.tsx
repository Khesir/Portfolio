...
export default function CmsExperienceEditor() {
	...
	const [jobType, setJobType] =
		useState<CreateExperienceDto['jobType']>('Remote');
	const [employmentType, setEmploymentType] =
		useState<CreateExperienceDto['employmentType']>('Full-time');
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (isEdit && id) {
				...
				toast.success(
					draft ? 'Experience saved as draft' : 'Experience published',
				);
			} else {
			...
			<form onSubmit={handleSubmit} className="space-y-5">
					...
					<div className="grid grid-cols-2 gap-4">
						<div className="space-y-1.5">
							...
							<select
								...
								value={jobType}
								onChange={(e) =>
									setJobType(e.target.value as CreateExperienceDto['jobType'])
								}
							>
								{JOB_TYPES.map((t) => (
						...
						<div className="space-y-1.5">
							...
							<select
								...
								onChange={(e) =>
									setEmploymentType(
										e.target.value as CreateExperienceDto['employmentType'],
									)
								}
							>
				...
				<div className="flex gap-3 pt-2">
					...
					<Button
						type="button"
						variant="outline"
						onClick={() => navigate('/cms/experiences')}
					>
						Cancel
					</Button>
				</div>
