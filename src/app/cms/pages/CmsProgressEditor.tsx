...
export default function CmsProgressEditor() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await cmsCreateProgress({ name, draft });
      toast.success(draft ? 'Progress entry saved as draft' : 'Progress entry created');
    ...
    <div className="max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button type="button" variant="outline" onClick={() => navigate('/cms/progress')}>Cancel</Button>
        </div>
      </form>
    </div>
  );
